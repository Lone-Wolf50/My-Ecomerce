...
function Homepage() {
  ...
  return (
    <div className="min-h-screen bg-black text-off-white">
      {/* HEADER: High z-index ensures it stays on top of search results */}
       <header className="fixed top-0 left-0 w-full z-50 bg-black/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-[1440px] mx-auto flex items-center justify-between px-8 py-6">
          <div className="flex items-center gap-2 shrink-0">
            ...
            <h1 className="text-2xl font-black tracking-widest text-primary-gold">LUXE</h1>
          </div>

          <nav className={`${isMenuOpen ? "flex flex-col absolute top-full left-0 w-full bg-black/95 p-6 border-b border-white/10" : "hidden"} md:static md:flex md:flex-row items-center gap-12`}>
            <Link className="text-[11px] font-bold uppercase tracking-[0.3em] hover:text-primary-gold transition-colors" to="/">Home</Link>
            <Link className="text-[11px] font-bold uppercase tracking-[0.3em] hover:text-primary-gold transition-colors" to="/shop/underarm">Shop</Link>
            <Link className="text-[11px] font-bold uppercase tracking-[0.3em] hover:text-primary-gold transition-colors" to="/reviews">Reviews</Link>
          </nav>
          ...
          <div className="flex items-center gap-4">
            <Link to="/cart" className="relative p-2 hover:text-primary-gold transition-colors">
              ...
              {cartCount > 0 && (
                <span className="absolute top-0 right-0 bg-primary-gold text-black text-[9px] font-black size-4 flex items-center justify-center rounded-full">{cartCount}</span>
              )}
            </Link>
      ...
      <main className="max-w-[1440px] mx-auto px-8 pt-32">
        {/* SEARCH: Lower z-index so navbar covers it on scroll */}
        <section className="mt-12 mb-12 flex justify-center relative z-[40]" ref={searchRef}>
          <div className="relative w-full max-w-2xl group">
            <input
              type="text"
              placeholder="Search name or category..."
              className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 focus:border-primary-gold/50 outline-none transition-all"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            {searchQuery && (
              <div className="absolute top-full left-0 w-full mt-2 bg-black border border-white/10 rounded-2xl overflow-hidden z-[60] shadow-2xl">
                {filteredProducts.map(p => (
                  <Link key={p.id} to={`/product/${p.id}`} className="flex items-center gap-4 p-4 hover:bg-white/5 border-b border-white/5">
...
<section className="py-16 md:py-28">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="relative group">
              <div className="aspect-[4/5] w-full rounded-[2.5rem] overflow-hidden bg-white/5 border border-white/10 p-4 backdrop-blur-md shadow-2xl">
            ...
            <div className="flex flex-col gap-10 lg:pl-16">
              <div className="flex flex-col gap-6">
                <span className="text-primary-gold font-bold tracking-[0.4em] uppercase text-[10px]">Exclusively Crafted</span>
                <h1 className="text-6xl md:text-8xl font-black leading-[1] tracking-tighter">The Essence <br /> <span className="text-primary-gold">of Elegance</span></h1>
                <p className="text-lg text-off-white/50 max-w-lg leading-relaxed font-light">Handcrafted for the modern woman who values timeless design and superior quality.</p>
              </div>
              <div className="flex flex-wrap gap-6">
                <Link to="/shop/underarm" className="bg-white/10 backdrop-blur-md border border-white/20 text-primary-gold px-12 py-5 rounded-2xl font-black text-[11px] tracking-[0.2em] uppercase hover:bg-primary-gold hover:text-black transition-all duration-500 shadow-xl">Shop Collection</Link>
                <button className="bg-white/5 backdrop-blur-md border border-white/10 text-white px-12 py-5 rounded-2xl font-bold text-[11px] tracking-[0.2em] uppercase hover:bg-white/20 transition-all duration-300">Our Story</button>
              </div>
            </div>
          </div>
        </section>
        {/* FEATURED COLLECTIONS: Restored "Explore More" */}
        <section className="py-20 border-t border-white/5">
          <h2 className="text-4xl font-black uppercase mb-12">Featured Collections</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {bagCategories.map((cat) => (
              <Link key={cat.id} to={`/shop/${cat.id}`} className="group">
                <div className="aspect-[3/4] rounded-[2rem] overflow-hidden bg-white/5 border border-white/10 p-2 mb-4">
                ...
                <div className="text-center space-y-2">
                  <h3 className="text-[11px] font-black uppercase tracking-widest">{cat.title}</h3>
                  {/* EXPLORE MORE - VISIBLE BY DEFAULT */}
                  <div className="flex items-center justify-center gap-2 text-primary-gold">
        ...
        {/* COMMUNITY SECTION (RESTORED) */}
        <section className="py-20 border-t border-white/5">
          <h3 className="text-2xl font-black uppercase tracking-widest text-center mb-12">The Community</h3>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            {communityImages.map((img) => (
              <div key={img.id} className="aspect-square rounded-2xl overflow-hidden border border-white/5 grayscale hover:grayscale-0 transition-all">
                <div className="w-full h-full bg-cover bg-center" style={{ backgroundImage: `url(${img.url})` }}></div>
              </div>
            ))}
          </div>
        </section>
        <footer className="w-full bg-[#050505] border-t border-white/5 py-20 mt-20">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-16">
            <div className="flex flex-col gap-8">
              <div className="flex items-center gap-2 text-primary-gold">
                ...
                <h2 className="text-2xl font-black tracking-widest uppercase">LUXE</h2>
              </div>
              <p className="text-xs text-off-white/40 leading-relaxed max-w-xs">Ethically crafted luxury since 2012.</p>
            </div>
            <div>
              <h4 className="font-black text-[10px] uppercase tracking-[0.3em] mb-8 text-primary-gold">Shop</h4>
              <ul className="flex flex-col gap-4 text-[13px] text-off-white/50">
                <li><Link to="/shop/underarm">New Arrivals</Link></li>
                <li><Link to="/shop/crossbody">Bestsellers</Link></li>
              </ul>
            </div>
            <div>
              <h4 className="font-black text-[10px] uppercase tracking-[0.3em] mb-8 text-primary-gold">Support</h4>
              <ul className="flex flex-col gap-4 text-[13px] text-off-white/50">
          ...
          <div className="mt-20 pt-10 border-t border-white/5 text-[9px] uppercase tracking-[0.4em] text-white/20">
